...
Lớp NguoiDung đóng gói thông tin đăng nhập và cung cấp hàm la_admin()
để frontend/backend kiểm tra quyền nhanh chóng.
"""
...
class NguoiDung:
    ...
    Thuộc tính:
        ten_dang_nhap (str): Tên đăng nhập (duy nhất).
        mat_khau      (str): Mật khẩu (ở đây dùng plaintext cho đơn giản,
                             trong thực tế nên băm bằng hashlib/bcrypt).
        ho_ten        (str): Họ tên đầy đủ để hiển thị.
    ...
    def la_admin(self):
        """
        Kiểm tra người dùng có phải là quản trị viên không.

        Trả về:
            True  nếu vai_tro == 'admin'.
            False nếu là nhân viên.
        """
        return self.vai_tro == 'admin'

    def chuyen_thanh_dict(self):
        """Chuyển đối tượng thành dict để trả về JSON (KHÔNG trả mật khẩu)."""
        return {
            'ten_dang_nhap': self.ten_dang_nhap,
            'ho_ten': self.ho_ten,
            'vai_tro': self.vai_tro,
            'la_admin': self.la_admin()
        }
...
def kiem_tra_dang_nhap(ten_dang_nhap, mat_khau):
    ...
    Tham số:
        ten_dang_nhap (str): Tên đăng nhập người dùng nhập.
        mat_khau      (str): Mật khẩu người dùng nhập.
