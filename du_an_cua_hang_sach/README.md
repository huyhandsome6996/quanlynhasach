...
│   ├── duong_dan.py                   # Điều hướng URL ứng dụng
│   ├── xu_ly_yeu_cau.py               # Xử lý yêu cầu HTTP (API endpoints)
│   │
│   └── loi_thuat_toan/               # Lõi thuật toán & cấu trúc dữ liệu
│       ├── __init__.py
│       ├── lop_doi_tuong.py           # Các lớp OOP (MatHang, Sach, TapChi...)
│       ├── danh_sach_lien_ket.py      # Danh sách liên kết đôi (DoublyLinkedList)
│       ├── cau_truc_du_lieu.py        # Stack (Undo/Redo) + Queue (Giỏ hàng)
│       ├── ket_noi_sqlite.py          # Kết nối cơ sở dữ liệu SQLite
